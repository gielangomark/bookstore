    ...
    <div class="relative bg-gray-50 min-h-screen">
        ...
        <div class="bg-indigo-600 pt-12 pb-8 px-6 md:px-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-3xl font-bold text-white">Edit Kategori</h2>
                    <p class="text-indigo-100 mt-1 text-sm md:text-base">Perbarui informasi kategori buku</p>
                </div>
            </div>
        </div>
                ...
                <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" 
                                  ...
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p class="text-sm text-blue-700">
                            ...
                            Kategori ini memiliki <strong>{{ $category->books_count ?? 0 }} buku</strong>
                        </p>
                    </div>
